...
<head>
    ...
    <title>User - Add New Event Post</title>
            ...
            <div class="content container-fluid">
                ...
                <div class="page-header">
                    <div class="row align-items-center justify-content-between">
                        ...
                        <div class="col-md-6">
                            <h3 class="page-title">Add New Event Post</h3>
                            <ul class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{url('user/dashboard')}}">Dashboard</a></li>
                                <li class="breadcrumb-item active">Add New Event Post</li>
                            </ul>
                        </div>
                    ...
                    <div class="col-lg-12 d-flex justify-content-center align-items-center">
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title text-muted">{{ $event->event_title }}</h5>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <form action="{{ route('eventPosts.store') }}" method="POST"
                                        ...
                                        <div class="mb-4 col-12 form-group">
                                            ...
                                            <ul id="tags-dropdown" class="list-group"
                                                style="display: none; position: absolute; z-index: 1000; background-color: white; border: 1px solid #ccc; max-height: 200px; overflow-y: auto;">
                                            </ul>
                                            @error('tags')
                                            <small class="text-danger">{{ $message }}</small>
                                            @enderror
                            ...
                            <div class="card-footer text-end d-flex justify-content-end" style="gap: 10px;">
                                <button type="submit"  name="status" value="1" class="btn btn-primary btn-lg fw-bold"
                                style="background: linear-gradient(to right, #007bff, #0056b3); border: none; color: #fff;">
                                <i class="fa fa-paper-plane me-2"></i> Publish
                                </button>
                                <button type="submit" name="status" value="2" class="btn btn-secondary">Save as Draft</button>
                            </div>
                            </form>
                        </div>
                    </div>
...
<script>
    ...
    function searchTags(query) {
        ...
        // Loop through all tags and check if the tag matches the query
        tags.forEach(function(tag) {
            var nameMatch = tag.tag.toLowerCase().includes(query); // Match tag field (adjusted to 'tag')
    ...
    function addTagToDisplay(tag) {
        ...
        // Collect tags to send to the hidden input field
        updateTagsField();
    }
    ...
    document.getElementById('tag-input').addEventListener('input', function() {
        var query = this.value.trim(); // Get the search query from the input field
        ...
        if (query.length > 0) {  // Search without checking for #
            ...
            if (results.length > 0) {
                results.forEach(function(tag) {
                    ...
                    resultItem.href = '#'; // No need to link for tags selection
                    resultItem.classList.add('list-group-item', 'list-group-item-action');
                    resultItem.textContent = tag.tag; // Show the tag field instead of 'name'
                    ...
                    resultItem.addEventListener('click', function(event) {
                        ...
                        // Add the tag to the display above the input (no hash)
    ...
    // To trigger the search again after a tag is selected and added

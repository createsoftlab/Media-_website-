...
<head>
    ...
    <title>User- My Event Posts</title>
            ...
            <div class="content container-fluid">
                 ...
                 <div class="page-header">
                    <div class="row align-items-center justify-content-between">
                        ...
                        <div class="col-md-6">
                            <h3 class="page-title">My Event Posts</h3>
                            <ul class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{ url('user/dashboard') }}">Dashboard</a></li>
                                <li class="breadcrumb-item active">My Events Posts</li>
                            </ul>
                        </div>
                    ...
                    <div class="border-0 shadow-sm card">
                        <div class="text-white card-header bg-dark">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div>
                                    <a class="btn btn-sm btn-danger" href="{{ route('user.draft.myEventPosts') }}">Drafts</a>
                                    <a class="btn btn-sm btn-danger" href="{{ route('user.bin.eventPost') }}">Bin</a>
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                                    ...
                                    <tbody id="post-table-body">
                                        @foreach ($posts as $post)
                                            <tr class="post-row" data-id="{{ $post->id }}" data-title="{{ $post->title }}">
                                                <td>{{ $post->id }}</td>
                                                <td>{{ Str::words($post->event_title, 4)}}</td>
                                                <td>{{ Str::words($post->title, 4) }}</td>
                                                <td class="text-center">
                                                    <img src="{{ asset('event_feature_image/'.$post->feature_image) }}" alt="{{ $post->title }}" class="img-thumbnail" style="width: 50px; height: 50px;">
                                                </td>
                                                <td class="text-center">
                                                    @if ($post->status == 1)
                                                    <span class="badge bg-success"
                                                        style="font-size: 0.8rem; padding: 0.6rem 0.6rem;">Active</span>
                                                    @else
                                                    <span class="badge bg-danger"
                                                        style="font-size: 0.8rem; padding: 0.6rem 0.6rem;">Offline</span>
                                                    @endif
                                                </td>
                                                <td class="text-center">
                                                    <a href="{{ route('user.edit.eventPost',$post->id) }}" class="text-primary me-4" style="font-size: 1.5rem;"><i class="la la-pencil"></i></a>
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="border-0 shadow-lg modal-content rounded-3">
                            <div class="p-5 modal-body">
                                ...
                                <form method="POST" action="{{ route('user.delete.eventPost',$post->id) }}">
                                    @csrf
                                    @method('PUT')

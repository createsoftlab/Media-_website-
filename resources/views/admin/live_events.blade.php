...
<head>
    ...
    <meta name="keywords" content="admin, events, bootstrap, business, corporate, creative, management, minimal, modern, accounts, invoice, html5, responsive, CRM, Projects">
    <meta name="author" content="Dreamguys - Bootstrap Admin Template">
    <meta name="robots" content="noindex, nofollow">
    <title>Admin - Live Events</title>
...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="page-wrapper">
                    ...
                    <div class="row align-items-center justify-content-between">
                        ...
                        <div class="col-md-6">
                            ...
                            <ul class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">Live Events</li>
                            </ul>
                        </div>
                    ...
                    <div class="border-0 shadow-sm card">
                        <div class="text-white card-header bg-dark">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <a class="btn btn-sm btn-danger" href="{{ route('add.liveevents') }}">Add New Live Event</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <!-- Search Input Table -->
                                ...
                                <table class="table mb-0 table-bordered table-striped table-hover">
                                    ...
                                    <tbody id="event-table-body">
                                        @foreach ($events as $event)
                                            <tr class="event-row" data-id="{{ $event->id }}" data-event_title="{{ $event->title }}" data-description="{{ $event->description }}">
                                                <td>{{ $event->title ?? 'N/A' }}</td>  <!-- Display title or 'N/A' if null -->
                                                <td>{{ \Illuminate\Support\Str::words($event->description, 5) ?? 'No description available' }}</td> <!-- Display first 5 words or a fallback message -->
                                                <td><a href="{{ $event->link }}" target="_blank">{{ $event->link ?? 'No link available' }}</a></td> <!-- Display link -->
                                                <td>
                                                    @if($event->image)
                                                        <img src="{{ asset('live_event_image/'.$event->image) }}" alt="{{ $event->title }}" class="img-thumbnail" style="width: 50px; height: 50px;">
                                                    @else
                                                        <span>No image available</span> <!-- Fallback if image doesn't exist -->
                                                    @endif
                                                </td>
                                                <td class="text-center">
                                                    <span class="badge bg-success">Active</span>  <!-- Assuming all events are active, but you can modify this based on your model -->
                                                </td>
                                                <td class="text-right">
                                                    <div class="dropdown dropdown-action">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-right">
                                                            <a class="dropdown-item" href="{{ route('edit.liveevents', $event->id) }}">
                                                                <i class="la la-pencil m-r-5"></i> Edit
                                                            </a>
                                                            <a class="dropdown-item" href="#" data-toggle="modal" data-target="#deleteModal_{{ $event->id }}">
            ...
            @foreach ($events as $event)
    <div class="modal custom-modal fade" id="deleteModal_{{ $event->id }}" tabindex="-1" role="dialog"
            ...
            <div class="border-0 shadow-lg modal-content rounded-3">
                <div class="p-5 modal-body">
                    <div class="mb-4 text-center">
                        ...
                        <p class="text-muted font-italic">Are you sure you want to delete this item? This action cannot be undone.</p>
                    </div>
                    <!-- Updated form action with the correct route -->
                    <form method="POST" action="{{ route('live_events.destroy', $event->id) }}">
                        @csrf
                        @method('DELETE') <!-- Make sure to specify DELETE method here -->
                        <div class="d-flex flex-column flex-sm-row justify-content-center">
                            <div class="mb-2 col-12 col-sm-5 mb-sm-0">
                                <button type="submit" class="py-3 shadow-sm btn btn-gradient-danger btn-lg w-100 hover-zoom-in">
                                    <i class="mr-2 fas fa-trash-alt"></i><span class="font-weight-bold">Yes, Delete it</span>
                                </button>
                            </div>
                            <div class="col-12 col-sm-5 offset-sm-1">
                                <button type="button" data-dismiss="modal"
                                    class="py-3 shadow-sm btn btn-light btn-lg w-100 hover-zoom-in">
                                    <i class="mr-2 fas fa-times-circle"></i><span class="font-weight-bold">No, Keep it</span>
                                </button>
                            </div>
                        </div>
                    </form>
